  ...
  var version = grunt.config.get("pasteup.version");
	...
	function buildDocumentationPages(cb) {
	  ...
	  async.forEach(fs.readdirSync('docs'), function(name, cb) {
	    ...
	    var f = grunt.template.process(ft.toString(), {'pasteupVersion': version});
	    var output = grunt.template.process(template, {'name':name, 'code':ft, 'pasteupVersion': version});
	...
	function buildModuleLibrary(cb) {
	  ...
	  async.forEach(fs.readdirSync('html/module'), function(name, cb) {
	    ...
	    var output = grunt.template.process(template, {'modules': modules, 'pasteupVersion': version});
	...
	function buildModulePages(cb) {
	  ...
	  async.forEach(fs.readdirSync('html/module'), function(name, cb) {
	    var module = grunt.file.read('html/module/' + name, 'utf8');
	    var output = grunt.template.process(template, {'name': name, 'code': module, 'pasteupVersion': version});
